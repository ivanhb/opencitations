...
format = as.data.frame(t(t(sapply(as.data.frame(t(background[,1:5])),sum))))
editor = as.data.frame(t(t(sapply(as.data.frame(t(background[,7:12])),sum))))
article = as.data.frame(t(t(sapply(as.data.frame(t(background[,13:13])),sum))))
version = as.data.frame(t(t(sapply(as.data.frame(t(background[,14:17])),sum))))

# Version 

print("Pearson with full SUS")
print(cor.test(article$V1,usersSUS$"SUS values"))

print("Pearson with Learnability")
print(cor.test(article$V1,usersLearn$"Learnability"))

print("Pearson with Usability")
print(cor.test(article$V1,usersUsability$"Usability"))

# Plot the diagrams of the aforementioned comparisons
par(mfrow=c(4,3))
plot(article$V1,usersSUS$"SUS values",xlab="Writing research articles experience score",ylab="SUS score",pch=16,col="blue",cex=2)
abline(lm(usersSUS$"SUS values"~article$V1), col="red", cex=5, lty=2)

plot(article$V1,usersLearn$"Learnability",xlab="Writing research articles experience score",ylab="Learnability score",pch=16,col="blue",cex=2)
abline(lm(usersLearn$"Learnability"~article$V1), col="red", cex=5, lty=2)

plot(article$V1,usersUsability$"Usability",xlab="Writing research articles experience score",ylab="Usability score",pch=16,col="blue",cex=2)
abline(lm(usersUsability$"Usability"~article$V1), col="red", cex=5, lty=2)
